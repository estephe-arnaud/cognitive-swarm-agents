# src/agents/agent_architectures.py
import logging
...
def create_research_planner_agent(llm: Optional[BaseLanguageModel] = None) -> AgentExecutor:
    if llm is None:
        llm = get_llm() # Utilise la version importée de get_llm
...
def create_synthesis_agent(llm: Optional[BaseLanguageModel] = None) -> AgentExecutor:
    if llm is None:
        # Utilise la version importée de get_llm et la constante importée SYNTHESIS_LLM_TEMPERATURE
